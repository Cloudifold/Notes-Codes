...
\begin{document}
    ...
    \begin{flushleft}
        Coordinate ring of $k$-variety $X$ : $k[X]$\\
        Category of commutative rings : $\mbf{CRing}$\\
        Jacobson radical of ring $R$ : $\Jac(R)$\\
        Polynomial ring with $n$-variables over ring $R$ : $R[x]_n$ or $R[x_1, \dotsc, x_n]$\\
        Radical ideal of ideal $I$ : $\sqrt{I}$\\
    ...
    \begin{sol}
        \
        \begin{prooftree}
            \AxiomC{$x \in \sqrt{\gen{0}}$}
            \UnaryInfC{$\E d \in \bbZ \ .\ x^{d} = 0$}
            \RightLabel{$w1$}
            \UnaryInfC{$(1+x)\cdot (\sum^{d-1}_{i=0}(-x)^{i})=1$}
            \UnaryInfC{$1+x \in \unitgrp{A}$}
            \AxiomC{$u \in \unitgrp{A}$}
            \UnaryInfC{$1+x \in \unitgrp{A} \leftrightarrow  u+ux \in \unitgrp{A}$}
            \AxiomC{$x \in \sqrt{\gen{0}}$}
            \UnaryInfC{$\A r \in A \ .\  rx \in \sqrt{\gen{0}}$}
            \RightLabel{$w2$}
            \TrinaryInfC{$ u+x \in \unitgrp{A}$}
        \end{prooftree}
        \begin{center}
        \begin{align*}
            & w1 : 1-x^n = (1+x)\cdot \left( \sum^{n-1}_{i=0}x^{i} \right) \\
            & w2 : u(1+\inv{u}x) = u+x,  \inv{u}x \in \unitgrp{A}
        \end{align*}
        \end{center}
        \qed
    \end{sol}
    ...
    \begin{sol}
        $$\Jac(A[x])=\bigcap_{m\in \Specmax(A[x])}m$$\\
        $$\sqrt{\gen{0}}_{A[x]}=\bigcap_{P\in \Spec(A[x])}P$$\\
        $$\Jac(A[x])\supseteq  \sqrt{\gen{0}}_{A[x]}$$\\
        $$q \in \Jac(A[x]) \leftrightarrow \A y \in A[x]\ .\ 1 - qy \in \unitgrp{A[x]}$$\\
        $c \in \unitgrp{A[x]} \to , c_0 \in \unitgrp{A} \ \rm{and}\  \A i\neq 0 \ .\ c_i \in \sqrt{\gen{0}}_A$\\
        $c \in \sqrt{\gen{0}}_{A[x]} \leftrightarrow  \A i \ .\ c_i \in \sqrt{\gen{0}}_A$\\
        $c=\sum_{i}c_i x^i \ (\A i \ .\ c_i \in A)$\\
        $q \in \Jac(A[x]) \to 1 - q \in \unitgrp{A[x]} \to \A i, q_i \in \sqrt{\gen{0}}_{A[x]} \to q \in \nilradical{A[x]}$\\
        \qed
    \end{sol}
