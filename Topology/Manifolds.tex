...
\begin{document}
    ...
    \begin{note}
        ...
        the \textit{pointwise converge} condition in the definition of $C^\omega$ function
        can be replaced equivalently by \textit{uniform converge}.
    \end{note}
    ...
    \begin{note}
        In definition above, the condition \textit{$\varphi_j(U_j)$ open in $\bbR^n$}
        can be replaced equivalently by \textit{$\varphi_j(U_j) = \bbR^n$}
        since we can use $\varphi_j^{-1}(B_r(\varphi_j(x)))$
        to produce a finer cover, and $\bbR^n$ is $C^\omega$ diffeomorphic to $B_r(y)$. 
        (by $ x \mapsto y + r \arctan(\norm{x}) \cdot x $)
    ...
    \begin{note}
        ...
        Submanifold is invariant under diffeomorphisms.\\
    \end{note}
    ...
    \begin{exmp}
        ...
        Suppose $E$ is spaned by $\mbfe_1, \ldots ,\mbfe_k$,
        $V$ have orthonormal basis $[\mbfv_1, \ldots , \mbfv_k]$
        and $V^\bot$ have orthonormal basis $[\mbfv_{k+1}, \ldots , \mbfv_{n}]$.
        Let $\GL_n(\bbR) \ni T : E \oplus E^\bot \to V \oplus V^\bot$
        be the transformation sends each $\mbfe_i$ to $\mbfv_i$.
        Let $[T]$ be its matrix respect to $[\mbfe_1, \mbfe_2, \ldots, \mbfe_n]$,
        and $[T^{-1}]$ is matrix of its inverse.
        then $p_{V^\bot}$
        is given by
        \[
        a = 
        [\mbfe_1, \mbfe_2, \ldots, \mbfe_n]
        \begin{bmatrix}
            a_1 \\
            a_2 \\
            \vdots \\
        ...
        \end{bmatrix}
        \mapsto
        [\mbfv_{k+1}, \mbfv_{k+2}, \ldots, \mbfv_n]
        \begin{bmatrix}
            T^{-1}
        \end{bmatrix}_{i > k}
        \begin{bmatrix}
    ...
    \begin{defn}
        Let $(M,A)$, $(N,B)$ be $C^r$-manifolds,
        a $C^r$ map $f : M \to N$
        is said to be a \tbf{($C^r$) local diffeomorphism at}
        $x \in M$
        if exists openset $U \ni x$,
        $f(U)$ is open in $Y$
        and $f|_U : U \to f(U)$ is a $C^r$-diffeomorphism.\\
        A $C^r$-map $f$ is said to be a \tbf{local diffeomorphism}
        if forall $x \in M$,
    ...
    \begin{thm}
        ...
        Suppose $W$ is open subset of $\bbR^m$,
        $f : W \to \bbR^n$ is a $C^r$ map.
        If $\d f_p : TW_p \to T\bbR^n_{f(p)}$
        is an isomorphism, then $f$
        is a local diffeomorphism at $p$.
    \end{thm}
    ...
    \begin{defn}
        Let $(M,A), (N,B)$ be two $C^r$-manifolds,
        and $f : M \to N$
        is an $C^{1}$ map.\\
        We call f is \tbf{immersive} at $x \in M$
        if $T_x f : T_x M \to T_x N$
        is monomorphism,
        and \tbf{submersive} if $T_x f$ is epimorphism.\\
        We call $f$ is an \tbf{immersion/submersion} if
        for all $x \in M$, $f$ is immersive/submersive at $x$.\\
        We call $f$ is an \tbf{embedding} if $f$ is immersion
        and $f : M \to f(M)$ is homeomorphism (between topological spaces).
    \end{defn}

    \begin{thm}
        $N$ is a $C^r$-manifold,
        A subspace $A \subseteq N$
        is a submanifold of $N$
        iff $A$ is the image of a $C^r$-embedding.
    \end{thm}
    ...
    \begin{prf}
        ...
        $\impliedby$ :
        Suppose $A = f(M)$,
        $\{(N_i, \varphi_i : N_i \to \bbR^n )\}_{i \in I}$ is charts of $N$ which covers $f(M)$.
        Then find $A_M := \{(M_i, \phi_i : M_i \to \bbR^k )\}_{i \in I}$,
        an atlas of $M$ such that $f(M_i) = A \cap N_i$.
        (we could find such $A_M$ since $f$ is an embedding)
        It is enough to prove that images of
        $C^r$ embeddings
        $f_i := \varphi_i \circ f \circ \phi_i^{-1} : \phi_i(M_i) \to \bbR^n$
        are submanifolds.
    \end{prf}
