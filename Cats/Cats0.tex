...
\begin{document}
    ...
    \begin{flushleft}
        Category of all small categories : $\mbf{Cat}$\\
        Terminal object of $\msC$: $\mbf{1}_{\msC}$\\
    \end{flushleft}
    ...
    \begin{defn}
        A 2-Cat  $\msA$  consists of :

        \begin{enumerate}
            \item A class $\abs{\msA}$
            \item for each $ X,Y \in \abs{\msA} $, a (small) category $ \Hom(X,Y) $
            \item for each $ X,Y,Z \in \abs{\msA} $, a bifunctor $ c_{X,Y,Z} : \Hom(X,Y) \times \Hom(Y,Z) \to \Hom(X,Z) $
            \item for each $ X \in \abs{\msA} $, a functor $ u_A : \mbf{1}_{\mbf{Cat}} \to \Hom(A,A) $
        \end{enumerate}
    ...
    \begin{flushleft}
    These data are required to satisfy following axioms:\\
    \end{flushleft}

    \begin{enumerate}
        \item Associativity axiom: for $ X, Y, Z, W \in \abs{\msA}$, \\$ c_{X,Z,W} \circ ( c_{X,Y,Z} \times \id_{\Hom(Z,W)} ) = c_{X,Y,W} \circ (\id_{\Hom(X,Y)} \times c_{Y,Z,W} )$
        \item Unit axiom : for $X, Y \in \abs{\msA}$, \\$ c_{X,X,Y} \circ (u_X \times \id_{\Hom(X,Y)}) = \id_{\Hom(X,Y)} = c_{X,Y,Y} \circ (\id_{\Hom(X,Y)} \times u_Y)$
    \end{enumerate}
